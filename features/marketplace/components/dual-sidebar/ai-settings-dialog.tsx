...
const PROVIDERS = [
  { name: "OpenRouter", value: "openrouter", endpoint: "https://openrouter.ai/api/v1" },
  { name: "Custom Endpoint", value: "custom", endpoint: "" },
];

// Popular AI models with user-friendly names and their slugs
            ...
            <>
              ...
              <div className="pb-4">
                ...
                <div className="relative mt-3">
                  <Input
                    id="provider"
                    placeholder="Select provider or enter custom endpoint"
                    value={localKeys.provider === "custom" ? localKeys.customEndpoint : PROVIDERS.find(p => p.value === localKeys.provider)?.name || localKeys.provider}
                    onChange={(e) => {
                      const value = e.target.value;
                      // If user types a URL, set as custom
                      if (value.startsWith("http://") || value.startsWith("https://")) {
                        setLocalKeys(prev => ({ ...prev, provider: "custom", customEndpoint: value }));
                      } else {
                        setLocalKeys(prev => ({ ...prev, provider: value, customEndpoint: "" }));
                      }
                    }}
                    className="w-full pr-10 shadow-sm"
                  />
                  <Popover open={isProviderDropdownOpen} onOpenChange={setIsProviderDropdownOpen}>
                    ...
                    <PopoverContent className="w-80 p-0" align="start">
                      <Command>
                        ...
                        <CommandList>
                          ...
                          <CommandGroup heading="Popular Providers">
                            {PROVIDERS.map((provider) => (
                              <CommandItem
                                ...
                                onSelect={() => {
                                  setLocalKeys(prev => ({
                                    ...
                                    customEndpoint: provider.value === "custom" ? prev.customEndpoint : ""
                                  }));
                              ...
                              >
                                <div className="flex flex-col">
                                  ...
                                  {provider.endpoint && (
                                    <span className="text-sm text-muted-foreground">{provider.endpoint}</span>
                                  )}
                                </div>
                              </CommandItem>
                            ))}
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  Choose a provider or enter a custom OpenAI-compatible API endpoint.
                </p>
                {localKeys.provider === "custom" && (
                  <Input
                    id="custom-endpoint"
                    placeholder="https://your-api-endpoint.com/v1"
                    value={localKeys.customEndpoint}
                    onChange={(e) => setLocalKeys(prev => ({ ...prev, customEndpoint: e.target.value }))}
              ...
              <div className="pb-4">
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    Enter any model slug or choose from popular options above.
                  </p>
                </div>
              ...
              <div className="pb-4">
                  ...
                  <Input
                    ...
                    type="password"
                    placeholder={
                      localKeys.provider === "openrouter" ? "sk-or-v1-..." :
                      localKeys.provider === "ai-gateway" ? "Your AI Gateway API key" :
                      "Your API key"
                    }
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    ...
                    {localKeys.provider === "ai-gateway" && (
                      <>Configure your gateway at <a href="https://dash.cloudflare.com/" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">Cloudflare Dashboard</a></>
                    )}
                    {localKeys.provider === "custom" && "Enter the API key for your custom endpoint"}
                  </p>
                </div>
